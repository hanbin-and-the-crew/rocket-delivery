...
| 단계 | 설명 | 실제 구현 내용 |
|-----|------|----------------|
| RED | 실패하는 테스트 작성 | `ProductTest.java` 작성<br>- `create_WithValidInput_ShouldSucceed()` 테스트 작성<br>- 유효성 검증 로직 미구현 → **테스트 실패** |
|GREEN| 최소한의 코드로 테스트 통과 | `Product.java` 도메인 엔티티 구현<br>- `Product.create()` 팩토리 메서드에 유효성 검증 추가<br>- 상품명, 가격, 카테고리 ID, 업체 ID, 허브 ID, 재고량 검증<br>- **테스트 통과** ✅ |
|REFACTOR | 코드 개선 및 검증 강화 | - ProductFixture 패턴 도입으로 테스트 데이터 생성 간소화<br>- @ParameterizedTest로 재고량 경계값 테스트 추가<br>- 논리적 삭제(Soft Delete) 기능 추가 |
...
단순히 의존성을 가짜로 만든다가 아니라, 테스트의 초점을 어디에 두느냐(행동 검증 vs 상태 검증)에 따라 Mock 사용 방식이 달라진다는 것을 체감함
...
| 단계 | 설명 | 실제 구현 내용 |
|-----|------|----------------|
| RED | 실패하는 테스트 작성 | `ProductServiceTest.java` 작성<br>- `createProduct_WithValidInput_ShouldSucceed()` 테스트 작성<br>- ProductService 미구현 → **테스트 실패** |
| GREEN | 최소한의 코드로 테스트 통과 | `ProductService.java` 구현<br>- `createProduct()` 메서드 구현<br>- Product.create() 호출 및 저장<br>- **테스트 통과**  |
| REFACTOR| 코드 개선 및 검증 강화 | - `validateCategory()` 메서드 분리<br>- ArgumentCaptor 추가로 Stock Cascade 검증<br>- ProductFixture 패턴 도입 |
...
| 단계 | 설명 | 실제 구현 내용 |
|------|------|----------------|
|  RED | 실패하는 테스트 작성 | `ProductJpaRepositoryTest.java` 작성<br>- Product 저장 시 Stock도 함께 저장되는지 검증<br>- Cascade 설정 없음 → **Stock 미저장, 테스트 실패** |
| GREEN | 최소한의 코드로 테스트 통과 | `Product.java` 엔티티 수정<br>- `@OneToOne(cascade = CascadeType.ALL)` 추가<br>- `@MapsId`로 Product-Stock ID 동기화<br>- **테스트 통과**  |
| REFACTOR| 코드 개선 및 검증 강화 | - `flush()`/`clear()` 패턴으로 영속성 컨텍스트 제어<br>- ProductFixture 도입으로 테스트 데이터 생성 간소화<br>- **Soft Delete 시 Stock 상태 문제 발견** <br>→ Product 삭제 시 Stock도 UNAVAILABLE로 변경 필요 |
...
| 단계 | 설명 | 실제 구현 내용 |
|-----|------|----------------|
| RED | 실패하는 테스트 작성 | `StockTest.java` 작성<br>- 재고 예약 기능 테스트 작성<br>- `Stock.reserve()` 메서드 미구현 → **테스트 실패**<br><br>```java<br>@Test<br>void reserve_WithSufficientStock_ShouldSucceed() {<br>    stock.reserve(30); // 실패<br>}<br>``` |
| GREEN | 최소한의 코드로 테스트 통과 | `Stock.java` 구현<br>- `reserve()`, `confirmReservation()`, `cancelReservation()` 메서드 구현<br>- 가용 재고 검증 로직 추가<br>- **테스트 통과** ✅ |
| REFACTOR | 코드 개선 및 검증 강화 | - 중복 검증 로직 추출 (validateReserveQuantity)<br>- 상태 전환 로직 캡슐화 (updateStatus)<br>- ProductFixture 패턴 적용<br>- **동시성 문제 발견** 🚨<br>→ @Version 낙관적 락 + @Retryable 적용<br>→ Testcontainers로 실제 DB 환경 동시성 검증 |

---

### 계층별 TDD 적용

* **도메인 테스트**: Stock 도메인 객체 검증 (StockTest.java)
...
<img width="1118" height="264" alt="image" src="https://github.com/user-attachments/assets/d29f413a-6ea4-412f-927f-df2069708230" />
